...
for num_of_arms in range(MIN_BAO_ARMS_INDEX, MAX_BAO_ARMS):
    ...
    os.makedirs(f"logs/arm_logs_500/", exist_ok=True)
    ...
    sleep(1)
    os.makedirs(f"db_snapshots/500/{PATH}/", exist_ok=True) # TODO: Delete later
    os.system(f"cp bao_server/bao.db db_snapshots/500/{PATH}/bao_snapshot_with_{num_of_arms}_arms.db")
    sleep(5)
    os.system(f"python3 combiner.py db_snapshots/500/bao_snapshot_with_{num_of_arms}_arms.db results/arm_results_500/bao_run_{num_of_arms}_arms.txt")
    sleep(1)
