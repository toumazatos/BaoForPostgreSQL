...
for num_of_arms in range(MIN_BAO_ARMS_INDEX, MAX_BAO_ARMS):
    ...
    os.makedirs(f"results/arm_results/{PATH}/", exist_ok=True) # TODO: Delete later
    ...
    os.system(f"python3 run_queries_new.py {WORKLOAD_FILE} 1 0 {num_of_arms} logs/arm_logs/bao_run_{num_of_arms}_arms_log.json | tee results/arm_results/{PATH}/bao_run_{num_of_arms}_arms.txt")
    os.system("sync")
    ...
    os.makedirs(f"db_snapshots{PATH}/", exist_ok=True) # TODO: Delete later
    os.system(f"cp bao_server/bao.db db_snapshots{PATH}/bao_snapshot_with_{num_of_arms}_arms.db")
    sleep(5)
    os.system(f"python3 combiner.py db_snapshots{PATH}/bao_snapshot_with_{num_of_arms}_arms.db results/arm_results/{PATH}/bao_run_{num_of_arms}_arms.txt")
